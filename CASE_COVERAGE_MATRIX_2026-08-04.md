# Big Fame 案例證據覆蓋矩陣

> 更新日期：2026-08-05  
> 用途：區分「已有可公開證據頁」、「只有產品／店型入口」與「仍需原始文件」；不把素材數量當成案例完成度。

## 一、目前覆蓋狀態

| TA／案例方向 | 目前可公開頁面 | 已能回答的問題 | 尚缺證據 | 判定 |
|---|---|---|---|---|
| 眼鏡品牌門市／展示掛勾 | `case-eyewear-2016`、`optical-hooks`、`display-hooks` | 展示方向、掛勾／背板系統、需求討論入口；案例頁另接 EYEHK 2025／2018 圖面作為相關產品證據，明確不宣稱為同一案例 SKU | 客戶授權、正式 SKU 對應、數量、交期、交付地 | 已有匿名證據入口與產品圖面橋接；非完整交付案例 |
| 3C／POS／模組化展示 | `case-modular-3c-store`、`pos-displays`、`modular-fixtures` | 客戶問題、展示櫃與模組方向、材料方向、易運輸／組裝、Big Fame 開發範圍 | 正式合約分工、數量、交期、最終交付條件 | 目前最完整的 POS／模組化證據案例 |
| 零售店面展示設備採購整合 | `case-retail-fixture-procurement-integration`、`procurement`、`display-hooks` | 匿名 TA MUJI Valencia 採購文件紀錄；多品類展示設備、尺寸／單位、Big Fame 文件整理與採購協調脈絡 | 客戶授權、正式合約角色、正式數量、價格、目的地、現行交期與最終交付結果 | canonical clean URL 已補齊來源紀錄；可作採購入口證據，仍非具名完整交付案例 |
| 旅宿家具／商業空間 | `case-boutique-hotel-furniture`、`case-japanese-wine-bar`、`case-urban-storage` | 空間問題、家具／展示方向、材料與部分開發範圍、匿名邊界 | 客戶授權、正式角色、數量、交期、交付地與成果 | 已有多個匿名證據頁；不宣稱具名成果 |
| Urban Warehouse／模組化儲物空間 | `case-urban-storage`、`modular-fixtures`、`custom-metal-parts` | 超過 50 個元件、約 3.4 m³ 標準單元、鋼板／鋼管／鋼線、粉體塗裝方向、K/D 結構、帶門與雙層儲物單元；來源記錄空間啟用與跨城市複製方向 | 客戶授權、Big Fame 正式合約分工、正式數量、交期、交付地、包裝／運費與公開成果數據 | 已完成匿名來源證據頁；可說明系統與使用結果，仍非正式完整交付案例 |
| 小型旅館客房／可變家具 | 尚未上線；候選 clean URL：`case-hotel-room-transformable-table` | 小客房、食飲放置空間、折疊／巢狀桌評估、旋轉盤轉出／收回機能 | 客戶授權、Big Fame 正式角色、尺寸、數量、交期、交付地、照片可公開性 | 已完成來源讀回；待授權與角色確認 |
| 壓克力／藥妝桌上展示 | `case-page-cosmetic-organizer`、`cosmetic-organizers` | 文件版本、尺寸、壓克力／實木、包裝、樣品／量產交期方向 | 客戶名稱、正式 MOQ、訂單數量、交付地、完成成果與公開授權 | 已有產品開發資料紀錄；不是完整客戶案例 |
| 美妝／髮品展示旋轉架與端架 | `case-hair-display-spinner-engineering`、`display-hooks`、`modular-fixtures` | 髮品盒尺寸需求、端架／雙面旋轉架、槽板、鋼線掛勾、壓克力招牌、共用框架、代表性尺寸與來源標示容量、樣品與平板運輸方向 | 客戶／品牌授權、正式訂單數量、價格、目的地、現行 SKU、正式交付與現行商務條件 | 已公開匿名工程／樣品紀錄；不是具名完整交付案例 |
| 服飾品牌門市 | `applications` 店型／專案選集、`case-apparel-2016`、`apparel-store-fixtures` | 3 張來源照片可辨識中島、壁面、吊掛、桌面配件與鞋類展示；店型入口可整理採購問題 | 客戶名稱、Big Fame 實際承擔範圍、正式規格、數量、交期、交付與照片授權 | 已有證據控制的照片紀錄；仍未達完整交付案例 |
| 防盜／展示五金 | `anti-theft-hooks`、`display-hooks`、`case-eyewear-2016` | 防盜需求、背板／安裝系統、SKU／圖面／樣品確認流程 | 已完成交付的公開案例、正式機構與數量 | 已有產品證據；案例證據待補 |
| 珠寶展示配件／ISLE | 尚未上線；來源為 2020 估價圖面 | 木／石材底盤、鐵線掛件、戒指台、展示組合；可核對部分尺寸、孔位、螺牙與數量標註 | 是否打樣／量產／交付、Big Fame 正式承擔範圍、公開授權、完整 MOQ／交期／交付地 | 已完成規格證據讀回；待內部階段與授權確認 |
| IVY／25×40 模組化展示系統 | `case-ivy-modular-system`、`modular-fixtures` | 25×40 系統、600 mm 組合尺寸、M6 組裝、Island Set 與多個元件圖面；可作為設計／採購討論起點 | 特定客戶、安裝、量產數量、交付地與成果數據 | 已有可追溯的系統開發／樣品圖面證據；非客戶交付案例 |
| 三耳機展示組／互動展示工程 | `case-headphone-display-set`、`pos-displays`、`modular-fixtures` | 客戶圖面、11-37607 專案／零件對應、展示外殼、金屬支撐、按鈕、影音播放器、通路版本、工廠組裝與 Packout 文件；公開頁新增可核對工程交付包 | 客戶授權、正式合約角色、訂單數量、交期、交貨地、實際出貨與安裝結果 | 已公開匿名工程紀錄；證據強於照片選集，但仍非具名完整交付案例 |
| 汽車零件展示架／On Time 歷史工程紀錄 | 尚未上線；候選匿名頁：`case-automotive-parts-rack` | 客戶相容性問題、Big Fame 設計回覆、旋轉架／四向洞洞板架、缺陷改善、組裝文件與最終設計素材 | 客戶公開授權、正式數量、交期、交付地、現場照片授權與正式合約範圍 | 目前最強的匿名工程／交付紀錄候選；仍不作具名客戶成果宣稱 |

## 二、案例完成判定規則

案例只有在來源至少能支持以下項目時，才可稱為「證據完整案例」：

1. 客戶或空間遇到的問題。
2. 使用的展示產品、家具或系統。
3. 可核對的材料／製程或設計方向。
4. Big Fame 實際承擔的範圍，並區分來源記錄與待確認內容。
5. 數量、交期、交付地若來源沒有，就明確標示未公開，不從照片推算。
6. 可公開程度、授權與匿名邊界。
7. 對應產品頁與詢問 CTA。

## 三、下一個案例工作順序

### 優先補服飾

從 `40_Projects_案例\10_零售門市與商業空間` 中尋找同時具備長敘述、產品／材質文件與可公開照片的服飾案；若只有 3D 示意圖，維持在候選狀態，不升格為交付案例。

### 持續補 POS／藥妝

3C 案例已可支撐 POS／模組化展示問題；藥妝方向目前由 PAGE 產品開發紀錄支撐。要新增藥妝交付案例，必須再取得能支持客戶問題、實際承擔與公開範圍的原始文件。

### 不應做的事

- 不把 `applications` 的店型選集當作完整交付案例。
- 不把產品目錄或產品照片當成客戶成果。
- 不用 3D 示意圖推定已量產、數量、承重或交付。
- 不為了補齊分類而公開未核准客戶名稱或數字。
