...
|---|---|---|---|
| 1. 統一正式網站架構 | 167 個 HTML 全站靜態檢查通過：JSON-LD 可解析、內容頁 H1=1、canonical 存在、內部不再使用 `.html`；公開 sitemap 96 URL 全部可連線；Git `main`、`draft` 與 `release/draft` 已由 `git ls-remote` 核對為同一 SHA | 部分完成 | Search Console 全量重新處理與索引狀態仍需觀察 |
...
- 本輪 Urban Warehouse 三語頁內容已寫入並完成公開 runtime 讀回；三個發布分支以 `git ls-remote` 核對相同。
...
- 版本文件：正式 URL runtime 已讀回；分支 SHA 僅作版本追蹤，不單獨代替公開驗收。
...
- Git `main`、`draft`、`release/draft` 已同步，並以 `git ls-remote` 核對三者相同。
